...
class JWSHeaderKid(Regexp):
    ...
    EXAMPLE = "did:bid:LjgpST2rjsoxYegQDRm7EL#keys-4"
    PATTERN = rf"^did:(?:key:z[{B58}]+|bid:[{B58}]{{21,22}}(;.*)?(\?.*)?#.+)$"
...
class IndyDID(Regexp):
    ...
    #PATTERN = rf"^(did:bid:)?[{B58}]{{21,22}}$"
    PATTERN = re.compile(rf"^(did:bid:)?[{B58}]{{21,22}}$")
