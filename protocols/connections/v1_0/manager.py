...
class ConnectionManager(BaseConnectionManager):
    ...
    async def create_invitation(
        ...
        ::

            {
                ...
                "label": "Alice",
                "did": "did:bid:QmWbsNYhMrjHiqZDTUTEJs"
            }
        ...
        if public:
            ...
            invitation = ConnectionInvitation(
                label=my_label, did=f"did:bid:{public_did.did}", image_url=image_url
            )
    ...
    async def create_request(
        ...
        elif connection.with_did:
            my_info = await wallet.get_local_did(connection.with_did)
    ...
    async def receive_request(
        ...
        if connection:
            ...
            if connection.is_multiuse_invitation:
                wallet = self._session.inject(BaseWallet)
                if connection.with_did is not None:
                    my_info = await wallet.get_local_did(connection.with_did)
                    conn = await self.find_connection(request.connection.did, my_info.did)
                    if conn is not None:
                        await conn.delete_record(self._session)
                else:
                    my_info = await wallet.create_local_did()
                keylist_updates = await mediation_mgr.add_key(
                    my_info.verkey, keylist_updates
    ...
    async def create_response(
        ...
        if connection.my_did:
            my_info = await wallet.get_local_did(connection.my_did)
        elif connection.with_did is not None:
            my_info = await wallet.get_local_did(connection.with_did)
            conn = await self.find_connection(connection.their_did, my_info.did)
            if conn is not None:
                await conn.delete_record(self._session)
        else:
            my_info = await wallet.create_local_did()
