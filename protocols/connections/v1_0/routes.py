...
class CreateInvitationQueryStringSchema(OpenAPISchema):
    ...
    with_did = fields.Str(
        description="Create invitation with specified DID", required=False
    )
...
async def connections_create_invitation(request: web.BaseRequest):
    ...
    with_did = request.query.get("with_did")
...
async def connections_receive_invitation(request: web.BaseRequest):
    ...
    try:
        ...
        connection = await connection_mgr.receive_invitation(
            invitation, auto_accept=auto_accept, alias=alias, mediation_id=mediation_id, with_did=with_did
        )
