...
def canon_did(uri: str) -> str:
    """
    Convert a URI into a DID if need be, left-stripping 'did:bid:' if present.
    ...
    if uri.startswith("did:bid:"):
        rv = uri[8:]
        if ok_did(rv):
            return rv
    raise ValueError(
        "Bad specification {} does not correspond to a bid DID".format(uri)
    )
...
def canon_ref(did: str, ref: str, delimiter: str = None):
    ...
    if ok_did(ref):  # e.g., LjgpST2rjsoxYegQDRm7EL
        return "did:bid:{}".format(did)

    if ok_did(resource(ref, delimiter)):  # e.g., LjgpST2rjsoxYegQDRm7EL#keys-1
        return "did:bid:{}".format(ref)

    if ref.startswith(
            "did:bid:"
    ):  # e.g., did:bid:LjgpST2rjsoxYegQDRm7EL, did:bid:LjgpST2rjsoxYegQDRm7EL#3
        rv = ref[8:]
        if ok_did(resource(rv, delimiter)):
            return ref
        raise ValueError("Bad URI {} does not correspond to a bid DID".format(ref))

    if urlparse(ref).scheme:  # e.g., https://example.com/messages/8377464
        return ref

    return "did:bid:{}{}{}".format(did, delimiter if delimiter else "#", ref)  # e.g., 3
