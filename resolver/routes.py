...
@docs(tags=["resolver"], summary="Retrieve doc resource for requested did url")
@match_info_schema(DIDMatchInfoSchema())
#@response_schema(ResolutionResultSchema(), 200)
async def dereference_did(request: web.Request):
    """Retrieve a did document resource."""
    context: AdminRequestContext = request["context"]
    ...
    try:
        ...
        if not key and not service:
           return web.json_response(result.serialize())  
        
        dereResult = {"keys":[], "services":[]}

        if key:
            for keyEntry in result.did_document["verificationMethod"]:
                if keyEntry["id"].endswith("#" + key):
                    dereResult["keys"].append(keyEntry)

        if service:
            for serviceEntry in result.did_document["service"]:
                if serviceEntry["id"].endswith("#" + service):
                    dereResult["services"].append(serviceEntry)

        return web.json_response(dereResult)  
