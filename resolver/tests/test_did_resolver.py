...
TEST_DID0 = "did:bid:Kkyqu7CJFuQSvBp468uaDe"
...
async def test_match_did_to_resolver_native_priority(profile):
    ...
    native = MockResolver(["bid"], native=True)
    non_native = MockResolver(["bid"], native=False)
...
async def test_match_did_to_resolver_registration_order(profile):
    registry = DIDResolverRegistry()
    native1 = MockResolver(["bid"], native=True)
    registry.register(native1)
    native2 = MockResolver(["bid"], native=True)
    registry.register(native2)
    non_native3 = MockResolver(["bid"], native=False)
    registry.register(non_native3)
    native4 = MockResolver(["bid"], native=True)
